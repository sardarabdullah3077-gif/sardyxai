...
app.post(["/api/chats/:id/messages", "/chats/:id/messages"], async (req, res) => {
  ...
  if (llmBase && llmKey) {
    ...
    const sysPrompt = `You are SARDYX AI, a premium autonomous AI agent created by Sardar Abdullah Fazal.${customInstructions ? ` Custom instructions: ${customInstructions}.` : ""}${memCtx ? ` User context: ${memCtx}.` : ""}`;
